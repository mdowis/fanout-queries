...
function extractFrom(context, values) {
  ...
  for (const value of values) {
    ...
    if (prompt === undefined) {
      ...
      if (found) {
        prompt = found;
      } else if (typeof context.adapter.extraPrompt === 'function') {
        // Some responses carry the prompt in a shape no path can express —
        // e.g. the user-authored message inside a whole conversation document.
        try {
          const fromAdapter = context.adapter.extraPrompt([value]);
          if (fromAdapter) prompt = fromAdapter;
        } catch (_) {
          /* an adapter quirk must not sink the extraction */
        }
      }
    }
